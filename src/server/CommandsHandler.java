 ...
 * 
 * When a client sends a command that requires parameters, the server responds with
 * a NEED_PARAMETER status and a ParameterRequest. The client collects the input and
 * sends it back. The server continues this dialog until all parameters are collected,
 * then executes the command and returns the result.
 */
public class CommandsHandler {
    ...
    private List<ParameterRequest> getRequiredParameters(Commands command, String args) {
        ...
        switch (command) {
            ...
            case ADD:
                params.add(new ParameterRequest("dragon_name", "Enter dragon name:", 
                    ParameterRequest.ParameterType.STRING, true));
                params.add(new ParameterRequest("dragon_x", "Enter X coordinate:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("dragon_y", "Enter Y coordinate:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("dragon_age", "Enter dragon age:", 
                    ParameterRequest.ParameterType.INTEGER, true));
                params.add(new ParameterRequest("dragon_weight", "Enter dragon weight:", 
                    ParameterRequest.ParameterType.LONG, true));
                params.add(new ParameterRequest("dragon_type", "Enter dragon type (WATER, FIRE, EARTH, AIR, or leave empty):", 
                    ParameterRequest.ParameterType.DRAGON_TYPE, false));
                params.add(new ParameterRequest("head_size", "Enter dragon head size:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("head_teeth", "Enter number of teeth in head:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                break;

            case UPDATE_BY_ID:
                params.add(new ParameterRequest("update_id", "Enter dragon ID to update:", 
                    ParameterRequest.ParameterType.LONG, true));
                params.add(new ParameterRequest("dragon_name", "Enter new dragon name:", 
                    ParameterRequest.ParameterType.STRING, true));
                params.add(new ParameterRequest("dragon_x", "Enter new X coordinate:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("dragon_y", "Enter new Y coordinate:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("dragon_age", "Enter new dragon age:", 
                    ParameterRequest.ParameterType.INTEGER, true));
                params.add(new ParameterRequest("dragon_weight", "Enter new dragon weight:", 
                    ParameterRequest.ParameterType.LONG, true));
                params.add(new ParameterRequest("dragon_type", "Enter new dragon type (WATER, FIRE, EARTH, AIR, or leave empty):", 
                    ParameterRequest.ParameterType.DRAGON_TYPE, false));
                params.add(new ParameterRequest("head_size", "Enter new dragon head size:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                params.add(new ParameterRequest("head_teeth", "Enter new number of teeth in head:", 
                    ParameterRequest.ParameterType.FLOAT, true));
                break;

            case REMOVE_BY_ID:
                params.add(new ParameterRequest("remove_id", "Enter dragon ID to remove:", 
                    ParameterRequest.ParameterType.LONG, true));
                break;
            ...
            case COUNT_GREATER_THAN_TYPE:
                params.add(new ParameterRequest("filter_type", "Enter dragon type (WATER, FIRE, EARTH, AIR):", 
                    ParameterRequest.ParameterType.DRAGON_TYPE, true));
                break;

            case FILTER_STARTS_WITH_NAME:
                params.add(new ParameterRequest("name_prefix", "Enter name prefix to filter:", 
                    ParameterRequest.ParameterType.STRING, true));
                break;

            case EXECUTE_SCRIPT:
                params.add(new ParameterRequest("script_path", "Enter script file path:", 
                    ParameterRequest.ParameterType.STRING, true));
                break;
        }
    ...
    private DragonType parseDragonType(String str) throws IllegalArgumentException {
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid dragon type. Must be one of: WATER, FIRE, EARTH, AIR");
        }
    }
     ...
     */
    private void executeCommand(ClientSession session, ServerContext context, String args) throws Exception {
        ...
        switch (command) {
            ...
            case SHOW:
                resultDragons.addAll(
                    collection.getStream().collect(Collectors.toList())
                );
            ...
            case REMOVE_GREATER: {
                ...
                saveCollection();
                output.append("Removed ").append(removed).append(" dragons greater than given.\n");
                break;
            }
            ...
            case REMOVE_LOWER: {
                ...
                int removed = before - after;
                saveCollection();
                output.append("Removed ").append(removed).append(" dragons lower than given.\n");
                break;
            }
            ...
            case COUNT_BY_TYPE: {
                ...
                output.append("Count of dragons with type ").append(type).append(": ").append(count).append("\n");
                break;
            }

            case COUNT_GREATER_THAN_TYPE: {
                DragonType type = (DragonType) params.get("filter_type");
                int count = (int) collection.countIf(d -> d.getType() != null && d.getType().compareTo(type) > 0);
                output.append("Count of dragons greater than type ").append(type).append(": ").append(count).append("\n");
                break;
            }
            ...
            case FILTER_STARTS_WITH_NAME: {
                ...
                output.append("Dragons with name starting with '").append(prefix).append("':\n");
     ...
     */
    private Dragon createDragon(String name, Float x, Float y, Integer age, Long weight, 
                                 DragonType type, Float headSize, Float teeth) throws Exception {
    ...
    private ApiCommand commandsToApiCommand(Commands cmd) {
        switch (cmd) {
            case HELP: return ApiCommand.HELP;
            case INFO: return ApiCommand.INFO;
            case ADD: return ApiCommand.ADD;
            case SHOW: return ApiCommand.SHOW;
            case UPDATE_BY_ID: return ApiCommand.UPDATE_BY_ID;
            case REMOVE_BY_ID: return ApiCommand.REMOVE_BY_ID;
            case CLEAR: return ApiCommand.CLEAR;
            case SAVE: return ApiCommand.SAVE;
            case EXECUTE_SCRIPT: return ApiCommand.EXECUTE_SCRIPT;
            case EXIT: return ApiCommand.EXIT;
            case REMOVE_GREATER: return ApiCommand.REMOVE_GREATER;
            case REMOVE_LOWER: return ApiCommand.REMOVE_LOWER;
            case HISTORY: return ApiCommand.HISTORY;
            case COUNT_BY_TYPE: return ApiCommand.COUNT_BY_TYPE;
            case COUNT_GREATER_THAN_TYPE: return ApiCommand.COUNT_GREATER_THAN_TYPE;
            case FILTER_STARTS_WITH_NAME: return ApiCommand.FILTER_STARTS_WITH_NAME;
