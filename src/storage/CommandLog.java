...
/**
 * Represents a single command entry in the Write-Ahead Log (WAL).
 * Contains metadata about a command modification to the collection.
 */
public class CommandLog implements Serializable {
    ...
    public String toString() {
        return String.format("[%d] TXN:%d CMD:%s STATUS:%s TIME:%s DRAGON:%s",
                logId, transactionId, command.name(), status, timestamp, 
                dragonData != null ? dragonData.substring(0, Math.min(30, dragonData.length())) : "null");
    }
}
